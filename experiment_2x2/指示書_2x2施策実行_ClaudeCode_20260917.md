...
> **2026-09-26 廃止：処置の正本は効果測定チャット 9/26 の処置仕様書（B-25拡張版）**
> （下の「150文字以内・段落ブロック1つ・箇条書きにしない」は経緯として残す。適用には使わない）
> 廃止の理由：実験結果を新規記事の標準に転用するため、処置は新規記事と同じ形で試す必要がある。150文字の段落で試して効果が出ても、新規記事の標準は D-39 のブロック形式なので、試した形と使う形が違ってしまう。効果が段落の内容によるのか箇条書きの構造によるのかも分からない。**測ったものをそのまま使えるように形を揃える**

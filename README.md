...
### 2026-09-14 — 日次LLM観測を毎日から火・木・土の週3日に変えた(`d18af8e`)

LLMO効果測定実験(47本)に Gemini の枠(20回/日)を回すため、日次7本の LLM 観測を
**火・木・土だけ**にした。ワークフロー自体は毎日走り、GA4 / GSC は毎日取る。
観測しない日の `daily_summary` の言及率は空欄(0% にしない)。

- 定義: `src/settings.py` の `DAILY_LLM_WEEKDAYS = (1, 3, 5)`。観測は 2026-09-15(火)から
- **週の観測日数は3日が正常**。`config/rules_thresholds.yaml` の `observation` に期待値を置き、
  週次所見は3日未満のときだけ「観測日数不足」と書く
- **前週比は観測の方式と日数が同じ週どうしでだけ比べる**。変更をまたぐ 2026-09-21 の週
  (今週3日・前週7日)は「観測方式の変更週のため前週比は参考値」として扱う(2026-09-21 追加)
- **`R-P2` の「3観測日連続」は、約1週間に相当する**(例: 火・木・土、木・土・火)。
  毎日観測していた 9/14 までは3日だった。同じ閾値でも、発火までにかかる日数は2倍以上になる
- 日次・月次の Gemini が再試行で枠を使うと、同じ日の実験は本数を減らして走る。実験は日次・月次の完了を待ち、その実消費を引いて本数を決める
  (2026-09-19 に日次が17回投げ、実験の10本が 429 になったため。2026-09-21 追加)

9/14 をまたぐ言及率・言及シェアの推移は、観測の頻度が変わったことを前提に読む。
...
**取り出し元未確認(フォームの送信先は Supabase)** — フォームは自作の多段フォームで、
送信先は Supabase の関数(/form_submit)。本番の Salesforce の Lead に届く経路があるかは
2026-09-25 時点で未確認(制作管制 便JZ)。この指標を読むコードは無く、LLMO実験の判定にも使わない。
...
| rule_id | 発火条件 |
|---------|---------|
| `R-P2` | 過去に言及実績があり、直近3観測日連続で mention=FALSE(証跡には**実際の連続日数**と開始日を記録)。観測は火・木・土なので3観測日 ≒ 約1週間(2026-09-14 まで毎日観測・3日) |
...
> **2026-12 に手読みと照合する(2026-09-01 の判断)。**
> この方式は「社名で節が始まる」という構造の仮定に乗っている。
> 3回分(2026-09/10/11)を回答本文と突き合わせ、外れていれば方式を変える。

### 取り下げURLの引用は日次と月次の両方で数える

`retired_urls` は日次では **E-1 だけ**、月次では **12本すべて**を見る
(月次は全プロンプトが自社について聞く面のため)。

日次だけでは足りない。`fsdg.jp` は日次 E-1 で一度も引用されず、
月次 M-4 で初めて出た。日次だけを見ていると**永久に0件と表示され、
「引用が止まった」と誤読する**。lk_events の `place` に
`日次 E-1 × A-011` / `月次 M-4 × —` の形で見た範囲を書くので、
0件の行でも「どこを見て0だったのか」が分かる。
